...
def dict_keyboard(items) -> InlineKeyboardMarkup:
    rows = [[InlineKeyboardButton(text=(f"🛠 {i.name}" if i.is_active else f"⏸ {i.name}"), callback_data=f"dict:item:{i.id}")] for i in items]
    rows.append([InlineKeyboardButton(text="➕ Добавить задачу", callback_data="dict:add")])
    return InlineKeyboardMarkup(inline_keyboard=rows)

@router.message(F.text.in_({"Справочник замен", "🧰 Список задач"}))
async def dictionary(message: Message, state: FSMContext, session: AsyncSession) -> None:
    await state.clear()
    await message.answer("🧰 Список задач для быстрого выбора:", reply_markup=dict_keyboard(await ReplacementItemRepository(session).list()))

@router.callback_query(F.data == "dict:add")
async def add_item_start(call: CallbackQuery, state: FSMContext) -> None:
    await state.set_state(ItemAdd.name); await call.message.answer("Название задачи:"); await call.answer()

@router.message(ItemAdd.name)
async def add_item_name(message: Message, state: FSMContext) -> None:
    await state.update_data(name=message.text or ""); await state.set_state(ItemAdd.description); await message.answer("Описание задачи:")
...
async def add_item_desc(message: Message, state: FSMContext, session: AsyncSession) -> None:
    ...
    await state.clear(); await message.answer("✅ Задача добавлена.")
...
async def item_actions(call: CallbackQuery, session: AsyncSession) -> None:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✏️ Название", callback_data=f"dict:name:{item.id}"), InlineKeyboardButton(text="📝 Описание", callback_data=f"dict:desc:{item.id}"), InlineKeyboardButton(text="↕️ Порядок", callback_data=f"dict:order:{item.id}")],
...
async def edit_name_start(call: CallbackQuery, state: FSMContext) -> None:
    ...
    await call.message.answer("Новое название задачи:"); await call.answer()
...
async def edit_desc_start(call: CallbackQuery, state: FSMContext) -> None:
    ...
    await call.message.answer("Новое описание задачи:"); await call.answer()
...
async def toggle_item(call: CallbackQuery, session: AsyncSession) -> None:
    item = await ReplacementItemRepository(session).get(int(call.data.split(":")[-1]))
    if item: item.is_active = not item.is_active; await call.message.answer("✅ Статус задачи изменён.")
    await call.answer()

@router.callback_query(F.data.startswith("dict:delete:"))
async def delete_item(call: CallbackQuery, session: AsyncSession) -> None:
    item = await ReplacementItemRepository(session).get(int(call.data.split(":")[-1]))
    if item: await session.delete(item); await call.message.answer("🗑 Задача удалена.")
    await call.answer()
